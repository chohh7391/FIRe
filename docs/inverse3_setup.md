...
> 이 teleoperator 패키지는 이제 **git submodule**이다
> (`src/FIRe/lerobot_teleoperators/lerobot_teleoperator_inverse3`,
> repo: <https://github.com/chohh7391/lerobot_teleoperator_inverse3>).
> FIRe를 `--recurse-submodules` 없이 clone 했다면 먼저 내려받는다:
>
> ```bash
...
Haply SDK(독점)는 submodule에 포함돼 있지 않다. 먼저 내려받아 `inverse3_bridge/sdk`에
둔 뒤 빌드한다 (자세한 내용은 submodule의 `SETUP.md` 참고):

```bash
cd src/FIRe/lerobot_teleoperators/lerobot_teleoperator_inverse3/src/lerobot_teleoperator_inverse3/inverse3_bridge
curl -L -o HardwareAPI.0.2.8.linux.tar.gz https://cdn.haply.co/r/33857671/HardwareAPI.0.2.8.linux.tar.gz
tar -xzf HardwareAPI.0.2.8.linux.tar.gz
mv HardwareAPI.0.2.8.linux sdk    # 압축 해제된 폴더명을 정확히 "sdk"로 변경
